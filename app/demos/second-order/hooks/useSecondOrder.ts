...
export function useSecondOrder(
    target: number,
    { frequency, damping, response }: SecondOrderParams
) {
    ...
    useEffect(() => {
        ...
        const update = (now: number) => {
            ...
            const xd = (target - previousTarget.current) / Math.max(dt, 0.0001);
            previousTarget.current = target;

            const y = position.current;
            const yd = velocity.current;

            const acceleration = (target + k3 * xd - y - k1 * yd) / k2;

            velocity.current = yd + acceleration * dt;
            position.current = y + velocity.current * dt;
